...
void dlio::OdomNode::getParams() {
  ...
  RCLCPP_INFO(get_logger(), "get the extrinsic_t [ %f, %f, %f]", baselink2imu_t[0], baselink2imu_t[1], baselink2imu_t[2]);
  ...
  RCLCPP_INFO(get_logger(), "get the extrinsic_t [ %f, %f, %f]", baselink2lidar_t[0], baselink2lidar_t[1], baselink2lidar_t[2]);
...
void dlio::OdomNode::getScanFromROS(const sensor_msgs::msg::PointCloud2::SharedPtr& pc) {
  ...
  // pc->header.stamp = rclcpp::Time(pc->header.stamp.sec/1000000000);
  // std::cout << "-------------" << original_scan_->points.size() << "-------------" << std::endl;
  ...
  // std::cout << std::setprecision(18) << this->scan_header_stamp.seconds() << std::endl;
  // this->scan_header_stamp = rclcpp::Time(pc->header.stamp.sec/1000000000);
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  if (this->sensor == dlio::SensorType::OUSTER) {
    ...
    extract_point_time = [&sweep_ref_time](boost::range::index_value<PointType&, long> pt)
      { 
        // std::cout << std::fixed << std::setprecision(18) << pt.value().timestamp << std::endl;
  ...
  for (auto it = points_unique_timestamps.begin(); it != points_unique_timestamps.end(); it++) {
    ...
    // std::cout << "------" << std::setprecision(18) << timestamps.back() << std::endl;
    // std::cout << "timestamps push back" << extract_point_time(*it) << std::endl;
  ...
  if (!this->first_valid_scan) {
    ...
    // std::cout << "---------aft deskew---------" << std::endl;
    return;
  }

  // std::cout << "bf second scan onwards" << std::endl;
  ...
  // std::cout << "aft integrateImu" << std::endl;
...
#pragma omp parallel for num_threads(this->num_threads_)
  ...
  this->deskewed_scan = deskewed_scan_;
  this->deskew_status = true;
  // std::cout << "aft second scan onwards" << std::endl;
}
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr pc) {
  ...
  // Preprocess points
  // std::cout << "bf preprocessPoints" << std::endl;
  this->preprocessPoints();
  // std::cout << "aft preprocessPoints" << std::endl;

  if (!this->first_valid_scan) {
    // std::cout << "first scan invalid" << std::endl;
    return;
  }
...
bool dlio::OdomNode::imuMeasFromTimeRange(double start_time, double end_time,
  ...
  // std::cout << "enter imuMeasFromTimeRange" << std::endl;
  if (this->imu_buffer.empty() || this->imu_buffer.front().stamp < end_time) {
    ...
    // std::cout << std::setprecision(16) << imu_buffer.front().stamp << "," <<
    // std::setprecision(16) << end_time << std::endl;
  ...
  // std::cout << "aft imuMeasFromTimeRange" << std::endl;
...
void dlio::OdomNode::buildSubmap(State vehicle_state) {
  ...
  if (this->submap_kf_idx_curr != this->submap_kf_idx_prev){
    ...
    //------------------gicp setInputTarget to the submap--------------------
